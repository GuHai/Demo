                ...
                <script type="text/html" id="template" data-url="/ijob/api/FullTimeController/myBrokerPersonDetail">
                    {{each list as post}}
                        <li class="ul-li">
                            ...
                            <div class="time">
                                <span class="entry">入职时间：{{post.entry |dateFormat:'yyyy-MM-dd'}}</span>
                                <span class="leave">离职时间：{{post.quit |dateFormat:'yyyy-MM-dd'}}</span>
                            </div>
                        </li>
                    {{/each}}
                </script>
...
<script>
    $("#template").loadData({condition:{type4:1}}).then(function (result) {
        console.log(result);
        var remarks = $(".remark") ;
        var more = true;
        function conceal() {
