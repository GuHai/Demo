                    ...
                    <div class="company-content clearfix">
                        <div class="disflex clearfix">
                            ...
                            <div class="company-content-msg ">
                                ...
                                <p>{{map.position.positionUser.mainName }}</p>
                                <%--<p>正在招聘：<em>{{map.count}}</em>个职位</p>--%>
                            </div>
                        </div>
                        <div class="visit_home">
                            <%-- isEdit 是否可编辑 --%>
                            <p>在招职位：<em>{{map.count}}</em>个</p>
                            <a href="javascript:void(0);" onclick="ijob.storage.set('isEdit',false);ijob.gotoPage({url:'/ijob/api/InformationController/h5/mine/examineUserInfo/'+ijob.location.get().lng+'/'+ijob.location.get().lat +'/{{map.position.userID}}'});">访问工作号</a>
                        </div>
                    </div>
                </div>
                <div class="warn">
...
<script>
    ...
    function checkPosition(positionID,forwardUser){
        var query = {
            condition:{
        ...
        $.ajax({
            url: '/ijob/api/PositionController/h5/qz/checkPosition',
    ...
    var resumeID ;
    if(ijob.storage.get("forwardUser")){
        checkPosition(ijob.storage.get("position.id"),ijob.storage.get("forwardUser"));
    }else{
        init();
    }
    function init(){
        $("#todayJob").loadData("该职位已经关闭或删除").then(function(result){
            ...
            if(ijob.storage.get("shareObj") != null){
                ...
                var desc  = "";
                try{
                    // desc="招聘人数："+result.data.list[0].position.recruitsSum/result.data.list[0].position.personNumDay+"人"+result.data.list[0].position.personNumDay+"天"+"\n"+"地点："+ijob.storage.get("shareObj").city+"\n时间："+template.dateFormat(result.data.list[0].position.recruitStartTime,'MM/dd')+"-"+template.dateFormat(result.data.list[0].position.recruitEndTime,'MM/dd');
                    desc="招聘人数："+result.data.list[0].position.personNumDay+"人/天"+"\n"+"地点："+ijob.storage.get("shareObj").city+"\n时间："+template.dateFormat(result.data.list[0].position.recruitStartTime,'MM/dd')+"-"+template.dateFormat(result.data.list[0].position.recruitEndTime,'MM/dd');
                }catch (err){
                    window.location.reload();
                }
            ...
            // 判断是否有标签
            var txt = $(".taglist").text();
            if( txt == 0 || txt == null ){
                $(".details-tag").css("padding","0")
            }
