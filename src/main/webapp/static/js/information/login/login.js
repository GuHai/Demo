...
function tlsGetUserSig(res) {
    ...
    if (res.ErrorCode == webim.TLS_ERROR_CODE.OK) {
        ...
        if (res.ErrorCode == webim.TLS_ERROR_CODE.SIGNATURE_EXPIRATION) {
           ...
           mui.alert("[" + res.ErrorCode + "]" + res.ErrorInfo);
        }
    }
}
...
function webimLogin() {
    webim.login(
        ...
        function (resp) {
            loginInfo.identifierNick = $("#IJobNickName").val();//设置当前用户昵称
            loginInfo.headurl = $("#IJobInfoHeadImg").val();//设置当前用户头像
            initDemoApp();
            applyAddFriend();
        },
        function (err) {
