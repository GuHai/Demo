        ...
        switch ($('#ggm_my_role').val()) {
            ...
            case '群主':
                if ($('#ggm_group_type').val() == 'Private') {
                   mui.alert('私有群不支持设置群成员角色操作');
                    return;
                }
                if (row.Role == '群主') {
                   mui.alert('你不能设置自己的群角色');
                } else {
        ...
        switch ($('#ggm_my_role').val()) {
            ...
            case '管理员':
                if (row.Role == '成员') {
                   ...
                   mui.alert('你不能设置群主或管理禁言时间');
                }
                break;
            case '群主':
                if (row.Role == '群主') {
                   mui.alert('你不能对自己设置禁言时间');
                } else {
...
var forbidSendMsg = function () {
    if (!webim.Tool.validNumber($('#fsm_shut_up_time').val())) {
       mui.alert('您输入的禁言时间非法,只能是数字(0-31536000)');
        return;
    }
