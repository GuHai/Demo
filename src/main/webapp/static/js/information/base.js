...
function selectApp() {
    ...
    if (appType == 1) {//测试应用
        ...
        if ($("#sdk_app_id").val().length == 0) {
          mui.alert('请输入sdkAppId');
            return;
        }
        if (!validNumber($("#sdk_app_id").val())) {
           mui.alert('sdkAppId非法,只能输入数字');
            return;
        }
        if ($("#account_type").val().length == 0) {
           mui.alert('请输入accountType');
            return;
        }
        if (!validNumber($("#account_type").val())) {
           mui.alert('accountType非法,只能输入数字');
            return;
        }

        loginInfo.sdkAppID = 1400070635;
        loginInfo.accountType = 22243;
    }
    //将account_type保存到cookie中,有效期是1天
    webim.Tool.setCookie('accountType', loginInfo.accountType, 3600 * 8760);
