...
*/
    ...
    var wrapper = {
		...
		ta:[
			'<div id="' + this.calendarId + '" class="ta_calendar ta_calendar2 cf"><div class="rx_date"><div class="rx_date_m">',
	...
	ta:[
		...
		'	<a class="opt_sel" id="'+ this.mOpts.compareTrigger +'" href="javascript:void(0);">',
        '		<i class="i_orderd"></i>',
        '	</a>',
		'</div>'
	]
...
pickerDateRange.prototype.checkDateRange = function(startYmd, endYmd) {
    ...
    if(eTime >= sTime) {
        ...
        if(maxEDate.getTime() < eTime) {
           mui.alert('结束日期不能大于：' + this.date2ymd(maxEDate).join('-'));
            return false;
        }
    } else {
        // 判断是否超过最大日期外
        ...
        maxEDate.setDate(maxEDate.getDate() - this.mOpts.dayRangeMax + 1);
        if(maxEDate.getTime() > eTime) {
           mui.alert('开始日期不能小于：' + this.date2ymd(maxEDate).join('-'));
            return false;
        }
    }
    return true;
