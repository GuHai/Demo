 ...
 * ydui main
 */
var queryParam = {condition:{
        parentID:'100000'
    }};
//用来保存用户选择的省份和城市信息。
var provinceID = 0,cityID = 0,isMore = true;
! function(window) {
    "use strict";
...
! function(window) {
    ...
    CitySelect.prototype.init = function() {
        ...
        console.log(options)
        options.provance = dict.YD.Province;
        options.city = dict.YD.City;
        options.area = dict.YD.Area;
    ...
    CitySelect.prototype.open = function() {
        ...
        if (defaultSet.provance) {
            ...
            $cityElement.find('.cityselect-nav a').eq(0).addClass('crt').html(dict.YD.Select);
        }
    ...
    CitySelect.prototype.createDOM = function() {
        ...
        _this.$cityElement = $('' +
            ...
            '           <p class="cityselect-title">'+dict.YD.Region+'</p>' +
            ...
            '           <li class="cityselect-item" id="province" data-url="/api/CityController/findList">' +
            '               <div class="cityselect-item-box"></div>' +
            '           </li>' +
            '           <li class="cityselect-item" id="city">' +
            '               <div class="cityselect-item-box"></div>' +
            '           </li>' +
            '           <li class="cityselect-item" id="area">' +
            '               <div class="cityselect-item-box"></div>' +
            '           </li>' +
            '        </ul>' +
            '</div>');
    ...
    CitySelect.prototype.setNavTxt = function(index, txt) {
        ...
        $nav.eq(index + 1).addClass('crt').html(dict.YD.Select);
    ...
    CitySelect.prototype.loadProvance = function() {
        ...
        queryParam.condition.parentID = '100000';
        $("#province").btPost(queryParam,function (result) {
            var tempList = result.data.list;
            $.each(result.data.list, function(k, v) {
                arr.push($('<a class="' + (v.cityName == _this.defaultSet.provance ? 'crt' : '') + '" href="javascript:;" data-id="'+v.id+'"><span>' + v.cityName + '</span></a>').data({
                    citys: tempList,
                    tag: 0
                }));
            });
    ...
    CitySelect.prototype.loadCity = function() {
        var _this = this;
        queryParam.condition.parentID = _this.$itemBox.eq(0).find('a.crt').data('id');
        console.log(queryParam.condition.parentID);
        if (queryParam.condition.parentID == undefined){
            queryParam.condition.parentID = "430000";
        }
        $("#province").btPost(queryParam,function (result) {
            //初始化城市信息 调用接口
            var cityData = result.data.list;
            //判断缓存中是否存在城市数据。并且用户是否没有改变自己所选择的省份。
            if( ijob.storage.get("citys")!=null&&_this.$itemBox.eq(0).find('a.crt').data('id')==provinceID){
                cityData = ijob.storage.get("citys");
            }
            var arr = [];
            ijob.storage.set("citys",cityData);
            $.each(cityData, function(k, v) {
                arr.push($('<a class="' + (v.cityName == _this.defaultSet.city ? 'crt' : '') + '" href="javascript:;" data-id="'+v.id+'"><span>' + v.cityName + '</span></a>').data({
                    citys: cityData,
                    tag: 1
                }));
            });
            _this.fillItems(1, arr);
            //在城市数据初始化之后，才将用户的选择的省份信息记忆住，否则容易出现数据混乱。
            provinceID = _this.$itemBox.eq(0).find('a.crt').data('id');
    ...
    CitySelect.prototype.loadArea = function() {
        var _this = this;
        //初始化区的信息 调用接口
        queryParam.condition.parentID = _this.$itemBox.eq(1).find('a.crt').data('id');

        if(queryParam.condition.parentID==undefined){
            queryParam.condition.parentID = '430100';
        }
        var arr = [];
        $("#province").btPost(queryParam,function (result) {
            //初始化城市信息 调用接口
            var areaData = result.data.list;
            if( ijob.storage.get("areas")!=null&&cityID==_this.$itemBox.eq(1).find('a.crt').data('id')){
                areaData = ijob.storage.get("areas");
            }
            ijob.storage.set("areas",areaData);
            $.each(areaData, function(k, v) {
                arr.push($('<a class="' + (v.cityName == _this.defaultSet.area ? 'crt' : '') + '" href="javascript:;"><span>' + v.cityName + '</span></a>').data({ tag: 2 }));
            });

            if (arr.length <= 0) {
                arr.push($('<a href="javascript:;"><span>'+dict.YD.All+'</span></a>').data({ tag: 2 }));
            }
            _this.fillItems(2, arr);
            //在城市数据初始化之后，才将用户的选择的省份信息记忆住，否则容易出现数据混乱。
            cityID = _this.$itemBox.eq(1).find('a.crt').data('id');
            if(cityID==undefined){
                cityID = '430100';
            }
        })
